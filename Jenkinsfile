pipeline {
    ...
    environment {
        DOCKER_IMAGE       = "vamsi0406/vamsi"
        DOCKER_CREDENTIALS = "vamsi"
        AWS_CREDS          = "aws-eks"
        AWS_REGION         = "eu-west-2"
        EKS_CLUSTER        = "team4-eks-cluster"  // Replace with your cluster name
        KUBE_NAMESPACE     = "team4-vamsi-namespace"
    }

    stages {
        stage('Checkout Code') {
            steps {
                git branch: 'main', url: 'https://github.com/Vamsi-c6/Spring_Boot_Application.git'
            }
        }
